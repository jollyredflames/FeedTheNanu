...
class SaveState {
    ...
    ArrayList<Point> positionMap;
    int score;
    int size; // although size can be determined from the positionMap,
    // this is to allow the 'load game' phase to easily grab the necessary details needed to display saved games.
    String localTime; // so saved games can also be identified by when they were saved.

    SaveState(Map<String, Point> positionMap, Point gap, int score, int size) {
